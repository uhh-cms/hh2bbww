...
class PlotMLResultsSingleFold(
    ...
    detailed = luigi.BoolParameter(
        default=False,
        description="Whether detailed plots should be run",
        significant=False,
    )
    ...
    def create_plots(self, data):
        ...
        if self.detailed:
            log_memory("Before plotting input features detailed")
            ...
            log_memory("After plotting input features detailed")
            data.train.cleanup()
            data.val.cleanup()
            data.test.cleanup()
            plt.close("all")
        ...
        if self.detailed:
            log_memory("Before plotting introspection per node")
            ...
            log_memory("After plotting introspection per node")
            plt.close("all")
            gc.collect()
