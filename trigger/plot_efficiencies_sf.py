...
def plot_efficiencies_sf(
    ...
    **kwargs,
    ...
    TODO.
    """
    hists_def = hists["default"]
    hists_sf = hists["default_incl_trigger"]

    remove_residual_axis(hists_def, "shift")
    remove_residual_axis(hists_sf, "shift")

    hists_def = apply_process_settings(hists_def, process_settings)
    variable_inst = variable_insts[0]
    hists_def = apply_variable_settings(hists_def, variable_insts, variable_settings)
    hists_def = apply_density_to_hists(hists_def, density)

    hists_sf = apply_process_settings(hists_sf, process_settings)
    hists_sf = apply_variable_settings(hists_sf, variable_insts, variable_settings)
    hists_sf = apply_density_to_hists(hists_sf, density)
    ...
    # loop over processes
    for proc_inst, myhist in hists_def.items():
        ...
        if "data" not in proc_inst.name:
            myhist = hists_sf[list(hists_sf.keys())[1]]  # horribly hardcoded
